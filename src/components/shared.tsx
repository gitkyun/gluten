// @ts-nocheck
// shared.tsx — shared Gluten components
// TODO: Remove @ts-nocheck and add explicit props interfaces per component.
// Tracking: phase 2 of TS migration (see commit history).
// Migrated from Gluten-ClaudeDesign/components.jsx. Internal symbols become
// module-scope; export block at the bottom replaces the legacy window.assign.

import React, { useState, useEffect, useRef, useMemo, useCallback } from 'react';
import type { CSSProperties, ReactNode } from 'react';
import { BAKERY_BY_ID } from '@/data/bakeries';

// Save (taste accumulation) context
const SaveContext = React.createContext<any>(null);

// ── Mood color system ──────────────────────────────────────────────────────
// All mood keys collapse to a single brand color. The 5-key shape is preserved
// only because Onboarding iterates Object.entries(MOOD_COLORS) to render the
// mood selector cards and references MOOD_COLORS.sweet / .specialty directly.
// Brand value mirrors tokens.json: --color-fill-brand-default (#fe5102).
const BRAND_HEX = '#fe5102';
const BRAND_SOFT = 'rgba(254, 81, 2, 0.10)';
const MOOD_COLORS = {
  sweet:     BRAND_HEX,
  savory:    BRAND_HEX,
  healthy:   BRAND_HEX,
...
const MOOD_SOFT = {
  ...
  savory:    BRAND_SOFT,
  healthy:   BRAND_SOFT,
  specialty: BRAND_SOFT,
  pixel:     BRAND_SOFT,
};
const getMoodColor = (_k) => BRAND_HEX;
const getMoodSoft  = (_k) => BRAND_SOFT;
...
function MediaSlot({
  ...
  const overlayStyle = overlay === 'dark'
    ? 'linear-gradient(180deg, rgba(0,0,0,0.08) 0%, rgba(0,0,0,0.08) 42%, rgba(0,0,0,0.72) 100%)'
    : overlay === 'side'
      ? 'linear-gradient(90deg, rgba(0,0,0,0.50) 0%, rgba(0,0,0,0.12) 62%, transparent 100%)'
      : overlay === 'none'
        ? 'none'
        : 'linear-gradient(180deg, rgba(255,255,255,0.04) 0%, rgba(0,0,0,0.03) 48%, rgba(0,0,0,0.34) 100%)';

  return (
    <div onClick={onClick} className={onClick ? 'lift' : ''} style={{ position: 'relative', aspectRatio, overflow: 'hidden', cursor: onClick ? 'pointer' : undefined, ...style }}>
      {imageUrl ? (
        <img
          src={imageUrl}
          alt={caption || ''}
          style={{
            ...
            objectFit: 'cover', display: 'block', ...imageStyle,
          }}
        />
      ) : (
        <Placeholder tone={tone} style={{ position: 'absolute', inset: 0, borderRadius: 'inherit' }} />
      )}
      {overlayStyle !== 'none' && (
        <div style={{ position: 'absolute', inset: 0, background: overlayStyle, pointerEvents: 'none' }} />
      )}
      {caption && (
        <div style={{
          position: 'absolute', left: 14, top: 14, zIndex: 3,
          padding: '5px 9px', borderRadius: 99,
          background: 'rgba(255,255,255,0.86)', color: 'var(--color-text-primary)',
...
function Chip({ label, active = false, variant = 'solid', size = 'medium', disabled = false, onClick, style = {}, leading }) {
  ...
  let background = 'var(--color-fill-surface-subtle)';
  ...
  if (active) {
    if (isOutlined) {
      ...
      background = 'var(--color-fill-surface-brand)';
  ...
  return (
    <button 
      ...
      style={{
        ...
        boxShadow: active && !isOutlined ? '0 10px 20px rgba(254,81,2,0.16)' : 'none',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow: '0 14px 34px rgba(19,15,13,0.13), inset 0 1px 0 rgba(255,255,255,0.9)',
        ...
        {tabs.map(t => {
          ...
          return (
            <button key={t.id} onClick={onClick} className="lift" style={{
              ...
              padding: '8px 2px', borderRadius: 16,
      ...
      <button onClick={onSearch} className="lift" style={{
        ...
        boxShadow: '0 14px 30px rgba(254,81,2,0.26), 0 3px 8px rgba(17,24,39,0.14)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          {tabs.map(t => {
            ...
            return (
              <button key={t.id} onClick={() => onTabChange(t.id)} style={{
                ...
                borderRadius: 8,
                background: active ? 'var(--color-fill-surface-default)' : 'transparent',
              }}>
                {t.label}
              </button>
            );
...
function MagazineKicker({ children, inverse = false, style = {} }) {
  return (
    <div style={{
      fontSize: 10.5,
      fontWeight: 800,
      ...
      color: inverse ? 'rgba(255,255,255,0.72)' : 'var(--color-text-brand)',
      ...style,
    }}>{children}</div>
  );
}

function CoverStoryCard({ item, kicker = '오늘의 커버', title, subtitle, note, onClick, action, style = {} }) {
  return (
    <article onClick={onClick} className="lift" style={{
      borderRadius: 28,
      overflow: 'hidden',
      background: 'var(--color-fill-surface-default)',
      boxShadow: '0 18px 42px rgba(19,15,13,0.14)',
      cursor: onClick ? 'pointer' : 'default',
      ...style,
    }}>
      <MediaSlot
        imageUrl={item?.coverImage}
        tone={item?.ph || 'ph-wheat'}
        aspectRatio="1 / 1.12"
        overlay="dark"
        caption={kicker}
      ...
      >
        ...
        {action && <div style={{ position: 'absolute', right: 16, top: 16 }}>{action}</div>}
      </MediaSlot>
      {note && (
        <div style={{
          ...
          <MagazineKicker>Editor note</MagazineKicker>
          <p style={{
            margin: 0,
            fontSize: 14,
            lineHeight: '22px',
...
function EditorialCard({ item, title, subtitle, meta, onClick, action, imageRatio = '4 / 3', style = {} }) {
  return (
    <article onClick={onClick} className={onClick ? 'lift' : ''} style={{
      borderRadius: 22,
      overflow: 'hidden',
      background: 'var(--color-fill-surface-default)',
      border: '1px solid var(--color-border-subtle)',
      boxShadow: '0 10px 26px rgba(19,15,13,0.07)',
      cursor: onClick ? 'pointer' : 'default',
      ...style,
    }}>
      <MediaSlot
        imageUrl={item?.coverImage || item?.imageUrl}
        tone={item?.ph || item?.tone || 'ph-cream'}
        aspectRatio={imageRatio}
        overlay="soft"
        style={{ borderRadius: 0 }}
      >
        {action && <div style={{ position: 'absolute', right: 10, top: 10 }}>{action}</div>}
      </MediaSlot>
      <div style={{ padding: 16 }}>
        {meta && <MagazineKicker style={{ marginBottom: 7 }}>{meta}</MagazineKicker>}
        <div style={{
          fontFamily: 'var(--font-display)',
          fontSize: 17,
          lineHeight: '23px',
...
function RankedFeatureCard({ rank, item, onClick, action, compact = false, style = {} }) {
  return (
    <article onClick={onClick} className="lift" style={{
      display: 'grid',
      gridTemplateColumns: compact ? '74px 1fr auto' : '92px 1fr auto',
      gap: 14,
      alignItems: 'center',
      padding: 12,
      borderRadius: 22,
      background: 'var(--color-fill-surface-default)',
      border: '1px solid var(--color-border-subtle)',
      boxShadow: '0 8px 22px rgba(19,15,13,0.07)',
      cursor: 'pointer',
      ...style,
    }}>
      <MediaSlot
      ...
      <div style={{ minWidth: 0 }}>
        <MagazineKicker>#{rank} featured</MagazineKicker>
        <div style={{
          marginTop: 5,
        ...
        <div style={{
          ...
          overflow: 'hidden',
          textOverflow: 'ellipsis',
        }}>{item?.region} · {item?.distance} · {item?.signature}</div>
      </div>
      {action}
    </article>
  );
...
function ArchiveCard({ title, subtitle, meta, tone = 'ph-stone', imageUrl, onClick, style = {} }) {
  return (
    <article onClick={onClick} className={onClick ? 'lift' : ''} style={{
      borderRadius: 24,
      overflow: 'hidden',
      background: 'var(--color-fill-surface-default)',
      border: '1px solid var(--color-border-subtle)',
      boxShadow: '0 10px 24px rgba(19,15,13,0.07)',
      cursor: onClick ? 'pointer' : 'default',
      ...style,
    }}>
      <MediaSlot imageUrl={imageUrl} tone={tone} aspectRatio="16 / 10" overlay="side" />
      <div style={{ padding: 16 }}>
        {meta && <MagazineKicker style={{ marginBottom: 6 }}>{meta}</MagazineKicker>}
        <div style={{ fontSize: 17, lineHeight: '23px', fontWeight: 800, color: 'var(--color-text-primary)' }}>{title}</div>
        {subtitle && <div style={{ marginTop: 6, fontSize: 12.5, lineHeight: '18px', color: 'var(--color-text-secondary)' }}>{subtitle}</div>}
      </div>
    </article>
  );
}

function FieldNoteCard({ avatar, title, subtitle, body, imageTone = 'ph-wheat', imageUrl, onClick, action, style = {} }) {
  return (
    <article onClick={onClick} className={onClick ? 'lift' : ''} style={{
      borderRadius: 24,
      background: 'var(--color-fill-surface-default)',
      ...
      <MediaSlot imageUrl={imageUrl} tone={imageTone} aspectRatio="4 / 3" overlay="soft" />
      <div style={{ padding: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {avatar}
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14.5, fontWeight: 800, color: 'var(--color-text-primary)' }}>{title}</div>
            {subtitle && <div style={{ marginTop: 2, fontSize: 11.5, color: 'var(--color-text-tertiary)' }}>{subtitle}</div>}
          </div>
          {action}
        </div>
        {body && <p style={{ margin: '13px 0 0', fontSize: 14, lineHeight: '22px', color: 'var(--color-text-secondary)' }}>{body}</p>}
      </div>
    </article>
  );
}
...
export {
  SaveProvider, useSave, SaveContext, SaveToast,
  Icon, Glass, Placeholder, MediaSlot, SaveStar, Chip,
  BottomNav, SearchBottomBar, TopBar, SectionHead, MoodLoz,
  CoverStoryCard, EditorialCard, RankedFeatureCard, ArchiveCard, FieldNoteCard, MagazineKicker,
  // Figma Design System Components
  Avatar, Thumbnail, Divider, Button, ContentBadge, ListCell, IconButton,
  AvatarGroup, Card, ListCard, Checkbox, Radio, Switch, Tab, Tabs,
  SegmentedControl, Select, Textfield, Textarea, Alert, Menu, Tooltip,
  PaginationDots, PageCounter, SnackBar,
  // Mood color system
