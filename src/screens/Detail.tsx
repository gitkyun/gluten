...
import {
  Tabs, Tab, Chip, Glass, Placeholder, MediaSlot, Icon, IconButton, SaveStar, SegmentedControl,
  Avatar, useSave, getMoodColor, getMoodSoft, getKeywordEmoji, MagazineKicker,
} from '@/components/shared';
import { BAKERIES, BAKERY_BY_ID } from '@/data/bakeries';
...
function ScreenDetail({ bakeryId, onClose, openDetail, openReviewWrite, openOverlay }) {
  ...
  useEffect(() => { setTab('home'); }, [bakeryId]);
  ...
  return (
    <div className="fade-enter" style={{
      ...
      <div ref={scrollerRef} className="scroll" style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <MediaSlot imageUrl={b.coverImage} tone={b.ph} aspectRatio="62 / 40" style={{
        ...
        <div style={{ padding: '16px 18px 0', background: '#fff' }}>
          <MagazineKicker>Place essay</MagazineKicker>
          {/* Name row */}
...
// ════════════════════════════════════════════════════════════════════════════
function TabPhoto({ b }) {
  const [cat, setCat] = useState('전체 248');
