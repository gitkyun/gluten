...
// @ts-nocheck
// Post Write, Review Function, Location Permission, Collection Detail
import { useState, useEffect } from 'react';
import {
  Glass, Placeholder, MediaSlot, SaveStar, Chip, TopBar, SectionHead, Avatar, Divider,
  Button, ContentBadge, IconButton, AvatarGroup, Card, Checkbox, Radio, Switch,
  Tabs, SegmentedControl, Textfield, Textarea, Alert, Menu, Tooltip,
  PaginationDots, PageCounter, SnackBar, Icon, useSave, MagazineKicker,
  getMoodColor, getMoodSoft, getKeywordEmoji,
  MOOD_COLORS, MOOD_SOFT, KEYWORD_EMOJIS,
  ...
  return (
    <>
      ...
      <div style={{
        ...
        boxShadow: '0 -18px 46px rgba(20,15,8,0.20)',
      }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <MagazineKicker style={{ marginBottom: 4 }}>Save to archive</MagazineKicker>
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {localCollections.map(c => (
                  <button key={c.id} onClick={() => onPickFolder(c.id)} className="lift" style={{
                    ...
                    <MediaSlot imageUrl={c.coverImage} tone={c.ph} overlay="none" style={{
...
function ScreenPostWrite({ onClose }) {
  ...
  const [photos, setPhotos] = useState(['ph-wheat']);
...
function ScreenReviewWrite({ bakeryId, onClose }) {
  ...
  const [mood, setMood] = useState(new Set());
...
// COLLECTION DETAIL
// ═══════════════════════════════════════════════════════════════════════════
function ScreenCollectionDetail({ collectionId, onClose, openDetail, openOverlay }: any) {
...
// (window exports moved to bottom of file)
...
function ScreenReviewDetail({ bakeryId, reviewIndex, onClose, openDetail }) {
  ...
  const [currentIndex, setCurrentIndex] = useState(reviewIndex || 0);
...
// (intermediate window export removed; consolidated at bottom)

// ═══════════════════════════════════════════════════════════════════════════
// COMMUNITY MAP OVERLAY
// ═══════════════════════════════════════════════════════════════════════════
function ScreenCommunityMap({ community, recommended, onClose, openDetail }) {
  const [selectedId, setSelectedId] = useState(recommended[0]?.id || null);
...
export {
  ...
  ScreenPostWrite, ScreenReviewWrite, ScreenLocationPermission, ScreenCollectionDetail,
  ScreenUserProfile, ScreenPostDetail, ScreenReviewDetail,
  ScreenCommunityDetail, ScreenCommunityMap, ScreenTasteQuiz, ScreenRankDetail,
};
