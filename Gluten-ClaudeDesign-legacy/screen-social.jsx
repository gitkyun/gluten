// screen-social.jsx — exploration-sharing feed (Threads/Wanted-style)
const { useState: useStateS } = React;

const SOCIAL_FILTERS = ['전체', '오늘 다녀온', '저장 모음', '동네 산책', '메뉴 발견', '분위기'];
...
function ScreenSocial({ openDetail, openOverlay }) {
  const [filter, setFilter] = useStateS('전체');
  ...
  return (
    <div className="scroll fade-enter" style={{
      ...
      <div style={{
        ...
        borderTop: '6px solid var(--color-background-alternative)',
      }}>
...
function Post({ post, openDetail, openOverlay, divider }) {
  const [liked, setLiked] = useStateS(false);
  ...
  return (
    <article style={{
      ...
      borderBottom: divider ? '1px solid var(--color-line-alternative)' : 'none',
    }}>
      {/* User row */}
...
function PostMedia({ post, onClick }) {
  const { images, layout } = post;
  if (layout === 'tall') {
    return <Placeholder tone={images[0]} onClick={onClick} style={{
  ...
  if (layout === 'large') {
    // grid carries the aspect-ratio so both unequal-width cells stay equal-height
    return (
      <div onClick={onClick} className="lift" style={{
        display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 4, cursor: 'pointer',
        aspectRatio: '201 / 160',
      }}>
        <Placeholder tone={images[0]} style={{ height: '100%', borderRadius: 18, overflow: 'hidden' }}/>
        <Placeholder tone={images[1]} style={{ height: '100%', borderRadius: 18, overflow: 'hidden' }}/>
      </div>
    );
  }
  if (layout === 'grid') {
    return (
      <div onClick={onClick} className="lift" style={{
        ...
        <Placeholder tone={images[0]} style={{
          gridRow: '1 / span 2', borderRadius: 18, overflow: 'hidden', height: '100%',
        }}/>
        <Placeholder tone={images[1]} style={{ borderRadius: 18, overflow: 'hidden', height: '100%' }}/>
        <Placeholder tone={images[2]} style={{ borderRadius: 18, overflow: 'hidden', height: '100%' }}/>
      </div>
    );
  }
  return null;
...
window.ScreenSocial = ScreenSocial;
