...
function ScreenNear({ openDetail }) {
  const [sheetMode, setSheetMode] = useStateN('mid'); // 'low' | 'mid' | 'high'
  const [filter, setFilter] = useStateN('전체');
  const [selectedMarker, setSelectedMarker] = useStateN(null);
        ...
        <Glass radius={32} intensity="strong" style={{
          ...
          <div style={{
            ...
            <h2 style={{
              ...
              {filter === '전체' ? (
                <>지금 주변 인기 <span style={{ color: 'var(--color-primary-normal)' }}>TOP {filteredBakeries.length}</span></>
              ) : (
                <>주변 {filter} <span style={{ color: 'var(--color-primary-normal)' }}>{filteredBakeries.length}곳</span></>
              )}
            </h2>
            <div style={{
