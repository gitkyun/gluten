...
const { useState: useStateOn, useEffect: useEffectOn } = React;
...
function ScreenOnboarding({ onComplete }) {
  const [step, setStep] = useStateOn('splash');
  const [userType, setUserType] = useStateOn(null);
  const [breadSel, setBreadSel] = useStateOn(new Set());
  const [moodSel, setMoodSel] = useStateOn(new Set());
  const [bizNewsList, setBizNewsList] = useStateOn([
...
function TypeCard({ icon, title, desc, tone, onClick }) {
  return (
    <button onClick={onClick} className="lift" style={{
      padding: 18, borderRadius: 20, border: '1px solid var(--color-line-normal)',
...
function OnBizNews({ onClose, onSubmit }) {
  const [title, setTitle] = useStateOn('');
  const [content, setContent] = useStateOn('');
  const [selectedTag, setSelectedTag] = useStateOn('📢 공지 사항');
